...
export function JourneyMaps() {
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="rounded-xl border border-cc-mid-gray/15 bg-cc-white p-6 shadow-sm">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <h2 className="text-xl font-bold text-cc-navy">{active.persona.name}</h2>
        ...
        <div className="flex flex-wrap gap-3">
          {active.stages.map((s) => {
            ...
            return (
              ...
              >
                <p className="text-xs font-bold uppercase text-cc-mid-gray">{s.shortLabel}</p>
                <p className="mt-1 font-semibold text-cc-navy">{s.title}</p>
                <p className="mt-2 text-xs leading-relaxed text-cc-dark-text/90">
                  {s.description}
                </p>
                {isCc ? (
                  <p className="mt-2 text-xs font-bold text-cc-navy">Clear Current intervention</p>
                ) : null}
              </div>
            )
          })}
